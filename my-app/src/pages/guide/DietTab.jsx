...
export default function DietTab({ darkMode, userId }) {
  const [diets, setDiets] = useState([]); // API에서 가져온 공식 식단
  ...
  const fetchData = async () => {
    try {
      ...
      // 공식 식단 가져오기
      const dietsRes = await fetch(getApiUrl('/api/guide/diets'));
      if (dietsRes.ok) {
      ...
      // 사용자 게시글 가져오기
      const postsRes = await fetch(getApiUrl('/api/guide/user-diets'));
      if (postsRes.ok) {
  ...
  const convertUserPostToPost = (post) => ({
    ...
    meals: post.data?.meals,
    calories: post.data?.calories,
  ...
  const handleLike = async (id) => {
    ...
    // 공식 식단 좋아요 (아직 API 없음)
    if (typeof id === 'string' && id.startsWith('diet-')) {
